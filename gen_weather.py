...
class Weather():
    def __init__(self):
        self.f_in = open("weather_in.txt","w",encoding="utf-8")
        self.f_out = open("weather_out.txt","w",encoding="utf-8")
        self.pattern_file = pd.read_excel("weather.xlsx")
    ...
    def genBase(self,n,arr = -1):
        ...
        f1 = pd.Series(pattern_file['f1']).dropna()
        weather = pd.Series(pattern_file['weather']).dropna()
        location = pd.Series(pattern_file['location']).dropna()
        ...
        for i in range(n):
            ...
            s,t = self.fill(s,t,"f1", f1[random.randint(0,len(f1)-1)])

            s,t = self.fill(s,t,"weather", weather[random.randint(0,len(weather)-1)],"weather")
            s,t = self.fill(s,t,"location", location[random.randint(0,len(location)-1)],"location")
            
            
            # print(s)
            # print(t)
            if arr != -1:
                arr.append((s,t,'weather'))
            else:
                self.f_in.write(s+'\n')
                self.f_out.write(t+'\n')
